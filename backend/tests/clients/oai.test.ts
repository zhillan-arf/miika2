...
// Simpler infer test
describe("Simple infer test", () => {
    test("Should return output text for simple input", async () => {
        const { simpleInfer } = await import("../../src/clients/oai.js");
        const inputs: Message[] = [{
            role: "user",
            content: "Hello, how are you?"
        }];
        const output = await simpleInfer(inputs);
